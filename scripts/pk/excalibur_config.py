...
class ExcaliburConfig(StrategyV2ConfigBase):
    ...
    stoch_fast_k: int = Field(40, client_data=ClientFieldData(is_updatable=True))
    stoch_slow_k: int = Field(8, client_data=ClientFieldData(is_updatable=True))
    ...
    rsi_major_spike_peak_threshold: Decimal = Field(64.0, client_data=ClientFieldData(is_updatable=True))
    rsi_major_spike_recovery_threshold: Decimal = Field(62.5, client_data=ClientFieldData(is_updatable=True))
    rsi_major_crash_bottom_threshold: Decimal = Field(36.0, client_data=ClientFieldData(is_updatable=True))
    rsi_major_crash_recovery_threshold: Decimal = Field(37.5, client_data=ClientFieldData(is_updatable=True))
