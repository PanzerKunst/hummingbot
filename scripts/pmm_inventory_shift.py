...
class PMMInventoryShift(ScriptStrategyBase):
    ...
    # Inventory params
    target_inv_ratio = 0.5
    current_inv_ratio = 0.5
    inventory_delta = 1
    inventory_scalar = 1
    ...
    def update_multipliers(self):
        ...
        base_bal = self.connector.get_balance(base)
        base_bal_in_quote = base_bal * self.connector.get_price_by_type(self.trading_pair, self.price_source)
        quote_bal = self.connector.get_balance(quote)
        self.current_inv_ratio = float(base_bal_in_quote / (base_bal_in_quote + quote_bal))
        delta = (self.target_inv_ratio - self.current_inv_ratio) / self.target_inv_ratio
        self.inventory_delta: float = max(-1.0, min(1.0, delta))
    ...
    def format_status(self) -> str:
        ...
        inventory_price_shift = Decimal(self.inventory_multiplier) * self.reference_price
        lines.extend([f"  Target Inventory Ratio: {self.target_inv_ratio:.4f} | Current Inv Ratio: {self.current_inv_ratio:.4f} | Inv Delta: {self.inventory_delta:.4f}"])
        lines.extend([f"  Inventory Multiplier (bps): {self.inventory_multiplier * 10000:.4f} | Inv Price Shift: {inventory_price_shift:.4f}"])
