...
class MerlinConfig(ControllerConfigBase):
    controller_name: str = "merlin"
    ...
    # Triple Barrier
    stop_loss_pct: float = Field(0.3, client_data=ClientFieldData(is_updatable=True))
    take_profit_pct: float = Field(0.3, client_data=ClientFieldData(is_updatable=True))
    filled_order_expiration_min: int = Field(20, client_data=ClientFieldData(is_updatable=True))

    # Technical analysis
    sma_short_length: int = Field(5, client_data=ClientFieldData(is_updatable=True))
    sma_long_length: int = Field(25, client_data=ClientFieldData(is_updatable=True))
    ...
    # Generate config file: create --controller-config generic.merlin
    # Start the bot: start --script v2_with_controllers.py --conf conf_v2_with_controllers_merlin.yml


class Merlin(ControllerBase):
    def __init__(self, config: MerlinConfig, *args, **kwargs):
    ...
    async def update_processed_data(self):
        ...
        candles_df.ta.sma(length=self.config.sma_short_length, append=True)
        candles_df.ta.sma(length=self.config.sma_long_length, append=True)
        candles_df.rename(columns={f"SMA_{self.config.sma_short_length}": "sma_short"}, inplace=True)
        candles_df.rename(columns={f"SMA_{self.config.sma_long_length}": "sma_long"}, inplace=True)
    ...
    def can_create_executor(self, active_executors: List[ExecutorInfo], side: TradeType) -> bool:
        ...
        if side == TradeType.SELL:
            did_cross = self.did_short_sma_cross_under_long()
        ...
        if side == TradeType.BUY:
            did_cross = self.did_long_sma_cross_under_short()
